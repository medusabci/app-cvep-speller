...
// c-VEP Speller (branch Patterns) (Unity app)
...
//      - v2.0 (28/04/2023):    Non-annoying patterns included
    ...
    void resetTrainMatrix()
    {
        ...
        for (int r = 0; r < matrixTrain.GetLength(0); r++)
        {
            for (int c = 0; c < matrixTrain.GetLength(1); c++)
            {
                ...
                matrixTrain[r, c].transform.GetChild(0).gameObject.SetActive(false);         // Pattern
                matrixTrain[r, c].transform.GetChild(1).GetComponent<Text>().color = defaultTextColor;  // Text
            }
        }
    }
    ...
    void resetTestMatrix()
    {
        ...
        for (int r = 0; r < matrixTest.GetLength(0); r++)
        {
            for (int c = 0; c < matrixTest.GetLength(1); c++)
            {
                ...
                matrixTest[r, c].transform.GetChild(0).gameObject.SetActive(false);          // Pattern
                matrixTest[r, c].transform.GetChild(1).GetComponent<Text>().color = defaultTextColor;   // Text
            }
        }
    } 
    ...
    void onScreenSizeChange(float width, float height)
    {
        ...
        for (int r = 0; r < matrixTest.GetLength(0); r++)
        {
            ...
            for (int c = 0; c < matrixTest.GetLength(1); c++)
            {
                ...
                RectTransform rtp = matrixTest[r, c].transform.GetChild(0).GetComponent<RectTransform>();
                rtp.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, cellSize);
                rtp.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, cellSize);

                // Adapt the text of each command label
                float st_ = cellSize / optimalCellSize;
                matrixTest[r, c].transform.GetChild(1).GetComponent<Text>().GetComponent<RectTransform>().localScale = new Vector3(st_, st_, 1f);
            }
        }
        ...
        for (int r = 0; r < trainRows; r++)
        {
            ...
            for (int c = 0; c < trainCols; c++)
            {
                ...
                RectTransform rtp = matrixTrain[r, c].transform.GetChild(0).GetComponent<RectTransform>();
                rtp.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, cellSize);
                rtp.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, cellSize);

                // Adapt the text of each command label
                float st_ = cellSize / optimalCellSize;
                matrixTrain[r, c].transform.GetChild(1).GetComponent<Text>().GetComponent<RectTransform>().localScale = new Vector3(st_, st_, 1f);
            }
        }        
    }
    ...
    void onParametersReady()
    {
        ...
        for (int r = 0; r < testNRow; r++)
        {
            ...
            for (int c = 0; c < testNCol; c++)
            {
                ...
                matrixTest[r, c].transform.GetChild(1).GetComponent<Text>().text = matrices.test[currentMatrixIdx].item_list[idx].text;     // Text
        ...
        for (int r = 0; r < trainNRow; r++)
        {
            ...
            for (int c = 0; c < trainNCol; c++)
            {
                ...
                matrixTrain[r, c].transform.GetChild(1).GetComponent<Text>().text = matrices.train[currentMatrixIdx].item_list[idx].label;      // Text
                ...
                for (int r = 0; r < matrixTrain.GetLength(0); r++)
                {
                    for (int c = 0; c < matrixTrain.GetLength(1); c++)
                    {
                        ...
                        if (value == 0)
                        {
                            ...
                            matrixTrain[r, c].transform.GetChild(0).gameObject.SetActive(false);     // Pattern    
                            matrixTrain[r, c].transform.GetChild(1).GetComponent<Text>().color = colorText0;    // Text
                        }
                        else
                        {
                            matrixTrain[r, c].GetComponent<Image>().color = colorBox1;
                            matrixTrain[r, c].transform.GetChild(0).gameObject.SetActive(true);      // Pattern 
                            matrixTrain[r, c].transform.GetChild(1).GetComponent<Text>().color = colorText1;    // Text
                        }
                    }
                }
                ...
                for (int r = 0; r < matrixTest.GetLength(0); r++)
                {
                    for (int c = 0; c < matrixTest.GetLength(1); c++)
                    {
                        ...
                        if (value == 0)
                        {
                            ...
                            matrixTest[r, c].transform.GetChild(0).gameObject.SetActive(false);                                        // Pattern
                            matrixTest[r, c].transform.GetChild(1).GetComponent<Text>().color = colorText0;     // Text
                        }
                        else
                        {
                            matrixTest[r, c].GetComponent<Image>().color = colorBox1;
                            matrixTest[r, c].transform.GetChild(0).gameObject.SetActive(true);                                        // Pattern
                            matrixTest[r, c].transform.GetChild(1).GetComponent<Text>().color = colorText1;     // Text
                        }
                    }
                }
